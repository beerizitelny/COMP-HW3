...
namespace ast {
    ...
    void ExpList::push_front(const std::shared_ptr<Exp> &exp) { exps.insert(exps.begin(), exp); }

    void ExpList::push_back(const std::shared_ptr<Exp> &exp) { exps.push_back(exp); }

    Call::Call(std::shared_ptr<ID> func_id, std::shared_ptr<ExpList> args) :
        Exp(), func_id(std::move(func_id)), args(std::move(args)) {}

    Call::Call(std::shared_ptr<ID> func_id) : Exp(), func_id(std::move(func_id)), args(std::make_shared<ExpList>()) {}
    ...
    void Statements::push_back(const std::shared_ptr<Statement> &statement) { statements.push_back(statement); }
    ...
    While::While(std::shared_ptr<Exp> condition, std::shared_ptr<Statement> body) :
        Statement(), condition(std::move(condition)), body(std::move(body)) {}
    ...
    void Formals::push_front(const std::shared_ptr<Formal> &formal) { formals.insert(formals.begin(), formal); }

    void Formals::push_back(const std::shared_ptr<Formal> &formal) { formals.push_back(formal); }
    ...
    void Funcs::push_front(const std::shared_ptr<FuncDecl> &func) { funcs.insert(funcs.begin(), func); }

    void Funcs::push_back(const std::shared_ptr<FuncDecl> &func) { funcs.push_back(func); }
